...
class UserTests(unittest.TestCase):
    ...
    def test_get(self):
        ...
        self.assertTrue(
            any(user.id == "dd2239f6-ddf1-4107-981a-4cf94e415794" for user in all_users)
        )
        single_user = next(
            user
        ...
        self.assertEqual(
            "2016-08-16T23:17:06Z", format_datetime(single_user.last_login)
        )
        self.assertEqual("alice cook", single_user.fullname)
        self.assertEqual("alicecook@test.com", single_user.email)

        self.assertTrue(
            any(user.id == "2a47bbf8-8900-4ebb-b0a4-2723bd7c46c3" for user in all_users)
        )
        single_user = next(
            user
            for user in all_users
    ...
    def test_get_by_id(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/dd2239f6-ddf1-4107-981a-4cf94e415794",
                text=response_xml,
            )
            single_user = self.server.users.get_by_id(
    ...
    def test_update_missing_id(self):
        ...
        self.assertRaises(
            TSC.MissingRequiredFieldError, self.server.users.update, single_user
        )

    def test_remove(self):
        with requests_mock.mock() as m:
            m.delete(
                self.baseurl + "/dd2239f6-ddf1-4107-981a-4cf94e415794", status_code=204
            )
    ...
    def test_add(self):
        ...
        with requests_mock.mock() as m:
            ...
            new_user = TSC.UserItem(
                name="Cassie", site_role="Viewer", auth_setting="ServerDefault"
            )
    ...
    def test_populate_workbooks(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/dd2239f6-ddf1-4107-981a-4cf94e415794/workbooks",
                text=response_xml,
            )
            single_user = TSC.UserItem("test", "Interactor")
            single_user._id = "dd2239f6-ddf1-4107-981a-4cf94e415794"
            ...
            self.assertEqual(
                "2016-07-26T20:34:56Z", format_datetime(workbook_list[0].created_at)
            )
            self.assertEqual(
                "2016-07-26T20:35:05Z", format_datetime(workbook_list[0].updated_at)
            )
            self.assertEqual(
                "ee8c6e70-43b6-11e6-af4f-f7b0d8e20760", workbook_list[0].project_id
            )
            self.assertEqual("default", workbook_list[0].project_name)
            self.assertEqual(
                "5de011f8-5aa9-4d5b-b991-f462c8dd6bb7", workbook_list[0].owner_id
            )
    ...
    def test_populate_workbooks_missing_id(self):
        ...
        self.assertRaises(
            ...
            self.server.users.populate_workbooks,
            single_user,
        )

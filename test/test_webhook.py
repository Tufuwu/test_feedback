...
class WebhookTests(unittest.TestCase):
    ...
    def test_test(self):
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/ee8c6e70-43b6-11e6-af4f-f7b0d8e20760/test",
                status_code=200,
            )
    ...
    def test_request_factory(self):
        ...
        webhook_item._set_values(
            "webhook-id", "webhook-name", "url", "api-event-name", None
        )
        webhook_request_actual = "{}\n".format(
            RequestFactory.Webhook.create_req(webhook_item).decode("utf-8")
        )
        self.maxDiff = None
        # windows does /r/n for linebreaks, remove the extra char if it is there
        self.assertEqual(
            webhook_request_expected.replace("\r", ""), webhook_request_actual
        )

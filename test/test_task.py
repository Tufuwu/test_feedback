...
GET_XML_NO_WORKBOOK = os.path.join(
    TEST_ASSET_DIR, "tasks_no_workbook_or_datasource.xml"
)
...
GET_XML_WITH_WORKBOOK_AND_DATASOURCE = os.path.join(
    TEST_ASSET_DIR, "tasks_with_workbook_and_datasource.xml"
)
GET_XML_DATAACCELERATION_TASK = os.path.join(
    TEST_ASSET_DIR, "tasks_with_dataacceleration_task.xml"
)
...
class TaskTests(unittest.TestCase):
    ...
    def test_delete(self):
        with requests_mock.mock() as m:
            m.delete(
                self.baseurl + "/c7a9327e-1cda-4504-b026-ddb43b976d1d", status_code=204
            )
    ...
    def test_get_materializeviews_tasks(self):
        ...
        with requests_mock.mock() as m:
            ...
            all_tasks, pagination_item = self.server.tasks.get(
                task_type=TaskItem.Type.DataAcceleration
            )
        ...
        self.assertEqual(
            parse_datetime("2019-12-09T22:30:00Z"), task.schedule_item.next_run_at
        )
    ...
    def test_delete_data_acceleration(self):
        with requests_mock.mock() as m:
            m.delete(
                "{}/{}/{}".format(
                    self.server.tasks.baseurl,
                    TaskItem.Type.DataAcceleration,
                    "c9cff7f9-309c-4361-99ff-d4ba8c9f5467",
                ),
                status_code=204,
            )
            self.server.tasks.delete(
                "c9cff7f9-309c-4361-99ff-d4ba8c9f5467", TaskItem.Type.DataAcceleration
            )

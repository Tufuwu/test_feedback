...
class ViewTests(unittest.TestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(
            "SafariSample/sheets/ENDANGEREDSAFARI", all_views[0].content_url
        )
        self.assertEqual(
            "3cc6cd06-89ce-4fdc-b935-5294135d6d42", all_views[0].workbook_id
        )
        self.assertEqual("5de011f8-5aa9-4d5b-b991-f462c8dd6bb7", all_views[0].owner_id)
        self.assertEqual(
            "5241e88d-d384-4fd7-9c2f-648b5247efc5", all_views[0].project_id
        )
        ...
        self.assertEqual(
            "6d13b0ca-043d-4d42-8c9d-3f3313ea3a00", all_views[1].workbook_id
        )
        self.assertEqual("5de011f8-5aa9-4d5b-b991-f462c8dd6bb7", all_views[1].owner_id)
        self.assertEqual(
            "5b534f74-3226-11e8-b47a-cb2e00f738a3", all_views[1].project_id
        )
        self.assertEqual(
            "2002-05-30T09:00:00Z", format_datetime(all_views[1].created_at)
        )
        self.assertEqual(
            "2002-06-05T08:00:59Z", format_datetime(all_views[1].updated_at)
        )
    ...
    def test_get_by_id(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/d79634e1-6063-4ec9-95ff-50acbf609ff5",
                text=response_xml,
            )
    ...
    def test_get_by_id_missing_id(self):
        self.assertRaises(
            TSC.MissingRequiredFieldError, self.server.views.get_by_id, None
        )
    ...
    def test_get_with_usage_and_filter(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "?includeUsageStatistics=true&filter=name:in:[foo,bar]",
                text=response_xml,
            )
            ...
            all_views, pagination_item = self.server.views.get(
                req_options=options, usage=True
            )
    ...
    def test_populate_preview_image_missing_id(self):
        ...
        self.assertRaises(
            ...
            self.server.views.populate_preview_image,
            single_view,
        )
    ...
    def test_populate_image(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/d79634e1-6063-4ec9-95ff-50acbf609ff5/image",
                content=response,
            )
            single_view = TSC.ViewItem()
            single_view._id = "d79634e1-6063-4ec9-95ff-50acbf609ff5"
    ...
    def test_populate_image_with_options(self):
        ...
        with requests_mock.mock() as m:
            ...
            req_option = TSC.ImageRequestOptions(
                imageresolution=TSC.ImageRequestOptions.Resolution.High, maxage=10
            )
    ...
    def test_populate_csv(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/d79634e1-6063-4ec9-95ff-50acbf609ff5/data?maxAge=1",
                content=response,
            )
            single_view = TSC.ViewItem()
            single_view._id = "d79634e1-6063-4ec9-95ff-50acbf609ff5"
    ...
    def test_populate_csv_default_maxage(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/d79634e1-6063-4ec9-95ff-50acbf609ff5/data",
                content=response,
            )
            single_view = TSC.ViewItem()
            single_view._id = "d79634e1-6063-4ec9-95ff-50acbf609ff5"
    ...
    def test_populate_image_missing_id(self):
        ...
        single_view._id = None
        self.assertRaises(
            TSC.MissingRequiredFieldError, self.server.views.populate_image, single_view
        )
    ...
    def test_populate_permissions(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/e490bec4-2652-4fda-8c4e-f087db6fa328/permissions",
                text=response_xml,
            )
            single_view = TSC.ViewItem()
    ...
    def test_update_tags(self):
        ...
        with requests_mock.mock() as m:
            m.put(
                self.baseurl + "/d79634e1-6063-4ec9-95ff-50acbf609ff5/tags",
                text=add_tags_xml,
            )
            m.delete(
                self.baseurl + "/d79634e1-6063-4ec9-95ff-50acbf609ff5/tags/b",
                status_code=204,
            )
            m.delete(
                self.baseurl + "/d79634e1-6063-4ec9-95ff-50acbf609ff5/tags/d",
                status_code=204,
            )
            m.put(
                self.baseurl + "/d79634e1-6063-4ec9-95ff-50acbf609ff5", text=update_xml
            )
            single_view = TSC.ViewItem()
            single_view._id = "d79634e1-6063-4ec9-95ff-50acbf609ff5"

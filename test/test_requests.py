...
class RequestTests(unittest.TestCase):
    ...
    def test_make_post_request(self):
        with requests_mock.mock() as m:
            ...
            self.assertEqual(
                resp.request.headers["x-tableau-auth"],
                "j80k54ll2lfMZ0tv97mlPvvSCRyD0DOM",
            )
    ...
    def test_internal_server_error(self):
        ...
        with requests_mock.mock() as m:
            m.register_uri(
                "GET",
                self.server.server_info.baseurl,
                status_code=500,
                text=server_response,
            )
            self.assertRaisesRegex(
                InternalServerError, server_response, self.server.server_info.get
            )
    ...
    def test_non_xml_error(self):
        ...
        with requests_mock.mock() as m:
            ...
            self.assertRaisesRegex(
                NonXMLResponseError, server_response, self.server.server_info.get
            )

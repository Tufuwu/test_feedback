...
class FlowTests(unittest.TestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(
            "2019-06-16T21:43:28Z", format_datetime(all_flows[0].created_at)
        )
        self.assertEqual(
            "2019-06-16T21:43:28Z", format_datetime(all_flows[0].updated_at)
        )
        self.assertEqual("Default", all_flows[0].project_name)
        self.assertEqual("FlowOne", all_flows[0].name)
        self.assertEqual(
            "aa23f4ac-906f-11e9-86fb-3f0f71412e77", all_flows[0].project_id
        )
        ...
        self.assertEqual(
            "2019-06-18T03:08:19Z", format_datetime(all_flows[1].updated_at)
        )
        self.assertEqual("Default", all_flows[1].project_name)
        self.assertEqual("FlowTwo", all_flows[1].name)
        self.assertEqual(
            "aa23f4ac-906f-11e9-86fb-3f0f71412e77", all_flows[1].project_id
        )
    ...
    def test_update(self):
        ...
        with requests_mock.mock() as m:
            m.put(
                self.baseurl + "/587daa37-b84d-4400-a9a2-aa90e0be7837",
                text=response_xml,
            )
            single_datasource = TSC.FlowItem(
                "test", "aa23f4ac-906f-11e9-86fb-3f0f71412e77"
            )
        ...
        self.assertEqual(
            "7ebb3f20-0fd2-4f27-a2f6-c539470999e2", single_datasource.owner_id
        )
    ...
    def test_populate_permissions(self):
        ...
        with requests_mock.mock() as m:
            ...
            self.assertEqual(
                permissions[0].grantee.id, "aa42f384-906f-11e9-86fc-bb24278874b9"
            )

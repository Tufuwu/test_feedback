...
class RequestOptionTests(unittest.TestCase):
    ...
    def test_filter_equals(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/workbooks?filter=name:eq:RESTAPISample",
                text=response_xml,
            )
    ...
    def test_filter_tags_in(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.baseurl + "/workbooks?filter=tags:in:[sample,safari,weather]",
                text=response_xml,
            )
            req_option = TSC.RequestOptions()
            req_option.filter.add(
    ...
    def test_filter_tags_in_shorthand(self):
        ...
        with requests_mock.mock() as m:
            ...
            matching_workbooks = self.server.workbooks.filter(
                tags__in=["sample", "safari", "weather"]
            )
    ...
    def test_double_query_params(self):
        with requests_mock.mock() as m:
            ...
            opts.sort.add(
                TSC.Sort(
                    TSC.RequestOptions.Field.Name, TSC.RequestOptions.Direction.Asc
                )
            )
            ...
            self.assertTrue(
                re.search(
                    "filter=tags%3ain%3a%5bstocks%2cmarket%5d", resp.request.query
                )
            )
    ...
    def test_filter_sort_legacy(self):
        ...
        with requests_mock.mock() as m:
            ...
            self.assertTrue(re.search("queryparamexists=true", resp.request.query))
            self.assertTrue(
                re.search("filter=tags:in:%5bstocks,market%5d", resp.request.query)
            )

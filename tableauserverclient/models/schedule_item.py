...
class ScheduleItem(object):
    ...
    def _parse_common_tags(self, schedule_xml, ns):
        if not isinstance(schedule_xml, ET.Element):
            schedule_xml = ET.fromstring(schedule_xml).find(
                ".//t:schedule", namespaces=ns
            )
    ...
    def from_element(cls, parsed_response, ns):
        ...
        for schedule_xml in all_schedule_xml:
            ...
            schedule_item = cls(
                name, priority, schedule_type, execution_order, interval_item
            )
    ...
    def _parse_interval_item(parsed_response, frequency, ns):
        ...
        interval_elems = parsed_response.findall(
            ".//t:intervals/t:interval", namespaces=ns
        )
        interval = []
    ...
    def _parse_element(schedule_xml, ns):
        ...
        interval_item = None
        frequency_detail_elem = schedule_xml.find(
            ".//t:frequencyDetails", namespaces=ns
        )
        if frequency_detail_elem is not None:
            interval_item = ScheduleItem._parse_interval_item(
                frequency_detail_elem, frequency, ns
            )

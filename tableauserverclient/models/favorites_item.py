...
class FavoriteItem:
    ...
    def from_response(cls, xml, namespace):
        ...
        for datasource in parsed_response.findall(
            ".//t:favorite/t:datasource", namespace
        ):
            fav_datasource = DatasourceItem("")
            fav_datasource._set_values(
                *fav_datasource._parse_element(datasource, namespace)
            )
            if fav_datasource:

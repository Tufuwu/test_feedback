...
class WorkbookItem(object):
    ...
    def _parse_common_tags(self, workbook_xml, ns):
        if not isinstance(workbook_xml, ET.Element):
            workbook_xml = ET.fromstring(workbook_xml).find(
                ".//t:workbook", namespaces=ns
            )
    ...
    def _parse_element(workbook_xml, ns):
        ...
        data_acceleration_elem = workbook_xml.find(
            ".//t:dataAccelerationConfig", namespaces=ns
        )
        if data_acceleration_elem is not None:
            data_acceleration_config = parse_data_acceleration_config(
                data_acceleration_elem
            )

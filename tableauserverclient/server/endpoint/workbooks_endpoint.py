...
class Workbooks(QuerysetEndpoint):
    ...
    @property
    def baseurl(self):
        return "{0}/sites/{1}/workbooks".format(
    ...
    def get_by_id(self, workbook_id):
        ...
        return WorkbookItem.from_response(
            server_response.content, self.parent_srv.namespace
        )[0]
    ...
    @api(version="3.5")
    def create_extract(
        self, workbook_item, encrypt=False, includeAll=True, datasources=None
    ):
        id_ = getattr(workbook_item, "id", workbook_item)
        url = "{0}/{1}/createExtract?encrypt={2}".format(self.baseurl, id_, encrypt)

        datasource_req = RequestFactory.Workbook.embedded_extract_req(
            includeAll, datasources
        )
    ...
    def update(self, workbook_item):
        ...
        return updated_workbook._parse_common_tags(
            server_response.content, self.parent_srv.namespace
        )
    ...
    def update_connection(self, workbook_item, connection_item):
        url = "{0}/{1}/connections/{2}".format(
            self.baseurl, workbook_item.id, connection_item.id
        )
        ...
        logger.info(
            "Updated workbook item (ID: {0} & connection item {1})".format(
                workbook_item.id, connection_item.id
            )
        )
        return connection
    ...
    def download(
        self, workbook_id, filepath=None, include_extract=True, no_extract=None
    ):
        if not workbook_id:
            error = "Workbook ID undefined."
            raise ValueError(error)
        ...
        with closing(
            ...
            with open(download_path, "wb") as f:
                for chunk in server_response.iter_content(1024):  # 1KB
                    f.write(chunk)
        logger.info(
            "Downloaded workbook to {0} (ID: {1})".format(download_path, workbook_id)
        )
    ...
    def populate_connections(self, workbook_item):
        ...
        logger.info(
            "Populated connections for workbook (ID: {0})".format(workbook_item.id)
        )
    ...
    def populate_preview_image(self, workbook_item):
        ...
        logger.info(
            "Populated preview image for workbook (ID: {0})".format(workbook_item.id)
        )
    ...
    def publish(
        ...
        try:
            ...
            if file_extension not in ALLOWED_FILE_EXTENSIONS:
                error = "Only {} files can be published as workbooks.".format(
        ...
        if file_size >= FILESIZE_LIMIT:
            logger.info(
                "Publishing {0} to server with chunking method (workbook over 64MB)".format(
                    workbook_item.name
                )
            )
        ...
        if as_job:
            ...
            logger.info(
                "Published {0} (JOB_ID: {1}".format(workbook_item.name, new_job.id)
            )
            return new_job
        else:
            new_workbook = WorkbookItem.from_response(
                server_response.content, self.parent_srv.namespace
            )[0]
            logger.info(
                "Published {0} (ID: {1})".format(workbook_item.name, new_workbook.id)
            )
            return new_workbook

...
class Favorites(Endpoint):
    @property
    def baseurl(self):
        return "{0}/sites/{1}/favorites".format(
    ...
    def get(self, user_item, req_options=None):
        ...
        user_item._favorites = FavoriteItem.from_response(
    ...
    def add_favorite_workbook(self, user_item, workbook_item):
        url = "{0}/{1}".format(self.baseurl, user_item.id)
        add_req = RequestFactory.Favorite.add_workbook_req(
            workbook_item.id, workbook_item.name
        )
        server_response = self.put_request(url, add_req)
        logger.info(
            "Favorited {0} for user (ID: {1})".format(workbook_item.name, user_item.id)
        )
    ...
    def add_favorite_view(self, user_item, view_item):
        ...
        server_response = self.put_request(url, add_req)
        logger.info(
            "Favorited {0} for user (ID: {1})".format(view_item.name, user_item.id)
        )

    @api(version="2.3")
    def add_favorite_datasource(self, user_item, datasource_item):
        url = "{0}/{1}".format(self.baseurl, user_item.id)
        add_req = RequestFactory.Favorite.add_datasource_req(
            datasource_item.id, datasource_item.name
        )
        server_response = self.put_request(url, add_req)
        logger.info(
            "Favorited {0} for user (ID: {1})".format(
                datasource_item.name, user_item.id
            )
        )
    ...
    def add_favorite_project(self, user_item, project_item):
        url = "{0}/{1}".format(self.baseurl, user_item.id)
        add_req = RequestFactory.Favorite.add_project_req(
            project_item.id, project_item.name
        )
        server_response = self.put_request(url, add_req)
        logger.info(
            "Favorited {0} for user (ID: {1})".format(project_item.name, user_item.id)
        )

    @api(version="2.0")
    def delete_favorite_workbook(self, user_item, workbook_item):
        url = "{0}/{1}/workbooks/{2}".format(
            self.baseurl, user_item.id, workbook_item.id
        )
        logger.info(
            "Removing favorite {0} for user (ID: {1})".format(
                workbook_item.id, user_item.id
            )
        )
    ...
    def delete_favorite_datasource(self, user_item, datasource_item):
        url = "{0}/{1}/datasources/{2}".format(
            self.baseurl, user_item.id, datasource_item.id
        )
        logger.info(
            "Removing favorite {0} for user (ID: {1})".format(
                datasource_item.id, user_item.id
            )
        )
        self.delete_request(url)

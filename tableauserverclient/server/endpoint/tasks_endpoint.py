...
class Tasks(Endpoint):
    ...
    def get(self, req_options=None, task_type=TaskItem.Type.ExtractRefresh):
        ...
        all_tasks = TaskItem.from_response(
            server_response.content, self.parent_srv.namespace, task_type
        )
    ...
    def delete(self, task_id, task_type=TaskItem.Type.ExtractRefresh):
        ...
        if not task_id:
            ...
            raise ValueError(error)
        url = "{0}/{1}/{2}".format(
            self.baseurl, self.__normalize_task_type(task_type), task_id
        )

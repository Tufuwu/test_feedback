...
class Endpoint(object):
    ...
    def _safe_to_log(server_response):
        ...
        if (
            server_response.headers.get("Content-Type", None)
    ...
    def _check_status(self, server_response):
        if server_response.status_code >= 500:
            ...
            try:
                raise ServerResponseError.from_response(
    ...
    def delete_request(self, url):
        ...
        self._make_request(
            self.parent_srv.session.delete, url, auth_token=self.parent_srv.auth_token
        )
        ...
        def wrapper(self, *args, **kwargs):
            ...
            for p in params_to_check:
                ...
                if server_ver < min_ver:
                    error = "{!r} not available in {}, it will be ignored. Added in {}".format(
                        p, server_ver, min_ver
                    )
                    warnings.warn(error)
            return func(self, *args, **kwargs)

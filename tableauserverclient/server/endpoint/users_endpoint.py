...
class Users(QuerysetEndpoint):
    ...
    def get(self, req_options=None):
        ...
        all_user_items = UserItem.from_response(
            server_response.content, self.parent_srv.namespace
        )
    ...
    def update(self, user_item, password=None):
        ...
        return updated_item._parse_common_tags(
            server_response.content, self.parent_srv.namespace
        )
    ...
    def add(self, user_item):
        ...
        new_user = UserItem.from_response(
            server_response.content, self.parent_srv.namespace
        ).pop()
    ...
    def populate_workbooks(self, user_item, req_options=None):
        ...
        def wb_pager():
            return Pager(
                lambda options: self._get_wbs_for_user(user_item, options), req_options
            )
    ...
    def _get_wbs_for_user(self, user_item, req_options=None):
        ...
        workbook_item = WorkbookItem.from_response(
            server_response.content, self.parent_srv.namespace
        )
        pagination_item = PaginationItem.from_response(
            server_response.content, self.parent_srv.namespace
    ...
    def _get_groups_for_user(self, user_item, req_options=None):
        ...
        group_item = GroupItem.from_response(
            server_response.content, self.parent_srv.namespace
        )
        pagination_item = PaginationItem.from_response(
            server_response.content, self.parent_srv.namespace

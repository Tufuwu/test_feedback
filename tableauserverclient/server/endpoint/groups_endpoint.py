...
class Groups(Endpoint):
    @property
    def baseurl(self):
        return "{0}/sites/{1}/groups".format(
    ...
    def get(self, req_options=None):
        ...
        all_group_items = GroupItem.from_response(
            server_response.content, self.parent_srv.namespace
        )
    ...
    def _get_users_for_group(self, group_item, req_options=None):
        ...
        server_response = self.get_request(url, req_options)
        user_item = UserItem.from_response(
            server_response.content, self.parent_srv.namespace
        )
        pagination_item = PaginationItem.from_response(
            server_response.content, self.parent_srv.namespace
    ...
    def update(self, group_item, default_site_role=None, as_job=False):
        ...
        if not group_item.id:
            ...
            raise MissingRequiredFieldError(error)
        if as_job and (
            group_item.domain_name is None or group_item.domain_name == "local"
        ):
        ...
        if as_job:
            return JobItem.from_response(
                server_response.content, self.parent_srv.namespace
            )[0]
        else:
            return GroupItem.from_response(
                server_response.content, self.parent_srv.namespace
            )[0]

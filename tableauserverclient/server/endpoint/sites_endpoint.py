...
class Sites(Endpoint):
    ...
    def get(self, req_options=None):
        ...
        all_site_items = SiteItem.from_response(
            server_response.content, self.parent_srv.namespace
        )
    ...
    def update(self, site_item):
        ...
        if site_item.admin_mode:
            if (
                site_item.admin_mode == SiteItem.AdminMode.ContentOnly
                and site_item.user_quota
            ):
                error = (
                    "You cannot set admin_mode to ContentOnly and also set a user quota"
                )
                raise ValueError(error)
        ...
        return update_site._parse_common_tags(
            server_response.content, self.parent_srv.namespace
        )
    ...
    def create(self, site_item):
        ...
        new_site = SiteItem.from_response(
            server_response.content, self.parent_srv.namespace
        )[0]

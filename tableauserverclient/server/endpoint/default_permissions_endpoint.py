...
class _DefaultPermissionsEndpoint(Endpoint):
    ...
    def update_default_permissions(self, resource, permissions, content_type):
        url = "{0}/{1}/default-permissions/{2}".format(
            self.owner_baseurl(), resource.id, content_type + "s"
        )
        update_req = RequestFactory.Permission.add_req(permissions)
        response = self.put_request(url, update_req)
        permissions = PermissionsRule.from_response(
    ...
    def delete_default_permission(self, resource, rule, content_type):
        for capability, mode in rule.capabilities.items():
            ...
            logger.debug(
                "Removing {0} permission for capabilty {1}".format(mode, capability)
            )
        ...
        logger.info(
            "Deleted permission for {0} {1} item {2}".format(
                rule.grantee.tag_name, rule.grantee.id, resource.id
            )
        )
    ...
    def populate_default_permissions(self, item, content_type):
        ...
        logger.info(
            "Populated {0} permissions for item (ID: {1})".format(item.id, content_type)
        )

    def _get_default_permissions(self, item, content_type, req_options=None):
        url = "{0}/{1}/default-permissions/{2}".format(
            self.owner_baseurl(), item.id, content_type + "s"
        )

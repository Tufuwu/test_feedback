...
class DataAlerts(Endpoint):
    ...
    @property
    def baseurl(self):
        return "{0}/sites/{1}/dataAlerts".format(
    ...
    def get(self, req_options=None):
        ...
        all_dataAlert_items = DataAlertItem.from_response(
            server_response.content, self.parent_srv.namespace
        )
    ...
    def get_by_id(self, dataAlert_id):
        ...
        return DataAlertItem.from_response(
            server_response.content, self.parent_srv.namespace
        )[0]
    ...
    def delete_user_from_alert(self, dataAlert, user):
        ...
        self.delete_request(url)
        logger.info(
            "Deleted User (ID {0}) from dataAlert (ID: {1})".format(
                user_id, dataAlert_id
            )
        )
    ...
    def add_user_to_alert(self, dataAlert_item, user):
        ...
        logger.info(
            "Added user (ID {0}) to dataAlert item (ID: {1})".format(
                user_id, dataAlert_item.id
            )
        )
        user = UserItem.from_response(
            server_response.content, self.parent_srv.namespace
        )[0]
        return user
    ...
    def update(self, dataAlert_item):
        ...
        updated_dataAlert = DataAlertItem.from_response(
            server_response.content, self.parent_srv.namespace
        )[0]

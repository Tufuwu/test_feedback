...
class Schedules(Endpoint):
    ...
    @property
    def siteurl(self):
        return "{0}/sites/{1}/schedules".format(
    ...
    def get(self, req_options=None):
        ...
        all_schedule_items = ScheduleItem.from_response(
            server_response.content, self.parent_srv.namespace
        )
    ...
    def update(self, schedule_item):
        ...
        return updated_schedule._parse_common_tags(
            server_response.content, self.parent_srv.namespace
        )
    ...
    def create(self, schedule_item):
        ...
        new_schedule = ScheduleItem.from_response(
            server_response.content, self.parent_srv.namespace
        )[0]
    ...
    def add_to_schedule(
        ...
        def add_to(resource, type_, req_factory):
            ...
            if task_created:
                logger.info(
                    "Added {} to {} to schedule {}".format(type_, id_, schedule_id)
                )
        ...
        if workbook is not None:
            items.append(
                (workbook, "workbook", RequestFactory.Schedule.add_workbook_req)
            )
        if datasource is not None:
            items.append(
                (datasource, "datasource", RequestFactory.Schedule.add_datasource_req)
            )

...
def main():
    ...
    parser.add_argument(
        "--workbook-name", "-w", required=True, help="name of workbook to move"
    )
    parser.add_argument(
        "--destination-site",
        "-d",
    ...
    with source_server.auth.sign_in(tableau_auth):
        ...
        if len(all_workbooks) == 0:
            ...
            try:
                workbook_path = source_server.workbooks.download(
                    all_workbooks[0].id, tmpdir
                )
                ...
                found_destination_site = any(
                    (
                        ...
                        if args.destination_site.lower() == site.content_url.lower()
                    )
                )
                ...
                with dest_server.auth.sign_in(tableau_auth):
                    ...
                    new_workbook = TSC.WorkbookItem(
                        name=args.workbook_name, project_id=""
                    )
